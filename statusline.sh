#!/usr/bin/env bash
# quotabar v1.2.3 — Claude Code statusline  (https://github.com/mangomandu/quotabar)
...
VER="1.2.3"   # 헤더의 'quotabar vX.Y.Z'와 동일하게 유지 — 업데이트 비교/표시에 사용
...
if [ "${1:-}" = "--update" ]; then
  ...
  if curl -fsSL "https://raw.githubusercontent.com/mangomandu/quotabar/main/statusline.sh" -o "$_tmp" 2>/dev/null && [ -s "$_tmp" ] && grep -q '^# quotabar v' "$_tmp" 2>/dev/null; then
    # 받은 파일이 진짜 quotabar 헤더를 가질 때만 덮어씀 — 404 HTML/끊긴 다운로드로 자기 자신을 깨먹지 않게
...
if [ "$ttl" -gt 0 ] && [ -z "$CC_USAGE_DEBUG" ] && [ -f "$cache.o" ] && [ -f "$cache.t" ] && [ ! "$conf" -nt "$cache.t" ]; then
  ...
  _now="${EPOCHSECONDS:-0}"; [ "$_now" = 0 ] && _now=$(date +%s 2>/dev/null || echo 0)  # bash5+ 빌트인, 아니면 date 폴백
...
if(env.CC_USAGE_DEBUG){
  ...
  // Codex 파일은 찾았는데 rate_limits를 못 뽑았으면 = rollout 포맷이 바뀐 신호일 수 있음 → 조용히 빈 바 대신 경고
  if(codexFile&&!Object.keys(cx).length)E.write("  WARNING Codex file found but no rate_limits parsed (rollout format change?): "+clean(codexFile)+"\n");
...
if [ "$ttl" -gt 0 ] && [ -n "$out" ]; then
  _wts="${EPOCHSECONDS:-0}"; [ "$_wts" = 0 ] && _wts=$(date +%s 2>/dev/null || echo 0)
