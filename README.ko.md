...
**성능/신선도**: `CC_USAGE_CACHE_TTL`(세션별 출력 재사용 초, 기본 `2`, `0`=비활성) · `CC_USAGE_STALE_MIN`(Codex 데이터가 N분 넘게 묵으면 마커, 기본 `30`, `0`=끔)
...
- **Codex 신선도**: Codex 값은 Codex가 마지막으로 실행된 시점 기준입니다(그때 데이터를 기록하니까). 리셋까지 남은 시간은 항상 정확하고, %만 마지막으로 알려진 값입니다. 데이터가 `CC_USAGE_STALE_MIN`(기본 30분)보다 묵었거나 윈도우가 이미 리셋됐으면, 해당 Codex 행에 흐린 `~경과` 마커(예: `~45m`, `~2h`)가 붙습니다.
