...
**솔직한 단점:** 렌더마다 `node`를 새로 띄우므로(~20ms 기동) *1회당*은 상주 앱의 인프로세스 틱보다 무겁습니다. 다만 훨씬 드물게 실행되고 유휴엔 전혀 안 돕니다. 연속 스트리밍 중에도 쓰로틀이 초당 몇 번으로 제한(1코어의 몇 %), 멈추면 즉시 0. 이 Node 기동 비용은 Node 기반 statusline(예: `ccusage`)이면 공통이고, quotabar는 거기에 더해 `ls`/`grep`/`tail` 서브프로세스까지 없앤 것뿐입니다.
