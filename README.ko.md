...
**성능/신선도**: `CC_USAGE_CACHE_TTL`(세션별 출력 재사용 초, 기본 `2`, `0`=비활성) · `CC_USAGE_STALE_MIN`(Codex가 N분 넘게 안 돌면 행을 `Cx idle ~Nh` 한 토막으로 접어 CC 뒤에 붙임, 기본 `30`, `0`=끔)
...
- **Codex 신선도**: Codex 값은 Codex가 마지막으로 실행된 시점 기준입니다(그때 데이터를 기록하니까). Codex가 `CC_USAGE_STALE_MIN`분(기본 30) 넘게 안 돌면, Codex 행이 `Cx idle ~경과` 한 토막으로 접혀 CC 뒤에 붙습니다(예: `… CC 7d … 74% · 1d2h   Cx idle ~4h`). 스테일한 Codex가 한 줄을 다 차지하거나 살아있는 척하지 않게 하기 위함입니다. quotabar가 Codex를 대신 갱신할 순 없고(그 데이터는 Codex가 돌 때만 기록됨), 그저 오래됨을 정직하게 알릴 뿐입니다.
